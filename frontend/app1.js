...
const tableConfigs = {
  ...
  bep: {
    ...
    columns: [
      ...
      { key: "naming", label: "Naming Convention", type: "textarea" },
...
function loadState() {
  // loadState tetap sync (baca localStorage sebagai cache awal).
  // Data DB dimuat async via initFromDB() saat startup.
  const fallbackProject = createDefaultProject("p_default", "Garage Premium");
...
async function initFromDB() {
  ...
  try {
    ...
    if (!projects.length) {
      // Belum ada project di DB → push project lokal ke DB
      const current = getProject();
      await apiPost("/api/projects", {
        id: current.id,
        projectInfo: current.projectInfo,
        activeDiscipline: current.activeDiscipline,
        autoCurveSync: current.autoCurveSync,
        uploadEndpoint: current.uploadEndpoint,
      });
      await syncToDB(current, false);
      toast("Project awal tersimpan ke database ✓");
      return;
    }

    // Load project aktif dengan semua tabel child dari DB
    const activeId = state.activeProjectId;
    const match    = projects.find(p => p.id === activeId) || projects[0];
    const full     = await apiGet("/api/projects/" + match.id);
    migrateProject(full);

    state.projects = projects.map(p => p.id === full.id ? full : p);
    state.activeProjectId = full.id;
...
function saveState(showToast = true) {
  // Selalu simpan ke localStorage sebagai backup lokal
  localStorage.setItem(STORE_KEY, JSON.stringify(state));
  // Jika DB tersedia, sync project aktif ke PostgreSQL
  if (USE_DB) {
    syncToDB(getProject(), false).then(() => {
      if (showToast) toast("Data tersimpan ke database ✓");
    });
  } else {
    if (showToast) toast("Data tersimpan (lokal)");
  }
}
...
function defaultRow(key) {
  ...
  if (key === "curve") {
    row.label = `Titik ${project.curve.length + 1}`;
    row.planned = 0;
    row.actual = 0;
    row.locked = true;
  }
...
async function saveRowModal(formData) {
  ...
  if (invalidCode) {
    toast("Kode deliverable sudah dipakai, gunakan nomor urut berbeda");
    return;
  }
...
async function deleteServerFile(serverUrl) {
  ...
  try {
    ...
    } else if (serverUrl.includes("/uploads/")) {
...
function renderTable(key, filteredRows = null) {
  ...
  const body = paged.map(({ row, index }) => {
    ...
    return `<tr data-row-id="${escapeHtml(String(rowId))}" data-row-no="${escapeHtml(String(row.no ?? index))}">${cells}<td class="actions-cell">
  ...
  }).join("") || `<tr><td colspan="${config.columns.length + 1}" class="empty-cell">Belum ada data. Klik tombol tambah.</td></tr>`;
...
// ── Kurva S: klik node → popup edit ──────────────────────
function openCurveNodePopup(nodeIdx, rect, nodeX, nodeY) {
  closeCurveNodePopup();
  const row = getProject().curve[nodeIdx];
  if (!row) return;
  const popup = document.createElement("div");
  popup.id = "curveNodePopup";
  popup.innerHTML = `
    <div class="cnp-head">
      <span class="cnp-title">✏️ Edit Titik Kurva</span>
      <button class="cnp-close" onclick="closeCurveNodePopup()">×</button>
    </div>
    <div class="cnp-body">
      <div class="cnp-field">
        <label>Periode</label>
        <input id="cnpLabel" type="text" value="${escapeHtml(row.label||"")}" placeholder="cth. Apr W1"/>
      </div>
      <div class="cnp-field">
        <label>Rencana Kumulatif (%)</label>
        <input id="cnpPlanned" type="number" min="0" max="100" step="0.5" value="${row.planned||0}"/>
      </div>
      <div class="cnp-field">
        <label>Aktual Kumulatif (%)</label>
        <input id="cnpActual" type="number" min="0" max="100" step="0.5" value="${row.actual||0}"/>
      </div>
      <div class="cnp-field cnp-field-check">
        <label class="cnp-check-label">
          <input id="cnpLocked" type="checkbox" ${row.locked?"checked":""}/>
          <span>Manual Lock</span>
        </label>
        <span class="cnp-check-hint">Centang agar nilai tidak berubah saat auto-sync</span>
      </div>
      <div class="cnp-actions">
        <button class="cnp-btn-cancel" onclick="closeCurveNodePopup()">Batal</button>
        <button class="cnp-btn-save" onclick="saveCurveNodePopup(${nodeIdx})">Simpan</button>
      </div>
    </div>`;
  const sx = window.scrollX||0, sy = window.scrollY||0;
  popup.style.cssText = `position:absolute;left:${rect.left+sx+nodeX+16}px;top:${rect.top+sy+nodeY-60}px;z-index:9999;`;
  document.body.appendChild(popup);
  requestAnimationFrame(() => {
    const r = popup.getBoundingClientRect();
    if (r.right  > window.innerWidth  - 12) popup.style.left = (rect.left+sx+nodeX-r.width -16)+"px";
    if (r.bottom > window.innerHeight - 12) popup.style.top  = (rect.top +sy+nodeY-r.height+16)+"px";
  });
  setTimeout(() => document.getElementById("cnpLabel")?.focus(), 80);
  popup.querySelectorAll("input[type=text],input[type=number]").forEach(inp => {
    inp.addEventListener("keydown", e => {
      if (e.key==="Enter")  saveCurveNodePopup(nodeIdx);
      if (e.key==="Escape") closeCurveNodePopup();
    });
  });
}

function closeCurveNodePopup() {
  const el = document.getElementById("curveNodePopup");
  if (el) el.remove();
}

function saveCurveNodePopup(nodeIdx) {
  const project = getProject();
  const row     = project.curve[nodeIdx];
  if (!row) return;
  const label   = document.getElementById("cnpLabel")?.value.trim();
  const planned = parseFloat(document.getElementById("cnpPlanned")?.value)||0;
  const actual  = parseFloat(document.getElementById("cnpActual")?.value) ||0;
  const locked  = document.getElementById("cnpLocked")?.checked||false;
  if (label!==undefined) row.label = label;
  row.planned = Math.min(100, Math.max(0, planned));
  row.actual  = Math.min(100, Math.max(0, actual));
  row.locked  = locked;
  closeCurveNodePopup();
  scheduleSave();
  redrawCharts();
  renderTable("curve");
  toast("Titik kurva diperbarui ✓");
}
...
function initCurveChartClick() {
  ["sCurve","sCurveOverview"].forEach(canvasId => {
    ...
    canvas.addEventListener("click", function(e) {
      ...
      if (closest) {
        const hy = Math.abs(my-closest.yPlanned)<Math.abs(my-closest.yActual)
          ? closest.yPlanned : closest.yActual;
        openCurveNodePopup(closest.idx, rect, closest.x, hy);
      } else {
        closeCurveNodePopup();
      }
    });
  ...
  document.addEventListener("click", e => {
    if (!e.target.closest("#curveNodePopup") &&
        e.target.id!=="sCurve" && e.target.id!=="sCurveOverview")
      closeCurveNodePopup();
  });
}
...
(async () => {
  try {
    ...
    // Pastikan halaman selalu tampil, tidak pernah blank
